...
class MEC:
    ...
    def step(self, action):
        ...
        # TODO : change it from discrete to countinues
        ...
        for ue_index in range(self.n_ue):
            ...
            ue_arrive_task_size = np.squeeze(
                self.arrive_task_size[self.time_count, ue_index]
            )
            ue_arrive_task_dens = np.squeeze(
                self.arrive_task_dens[self.time_count, ue_index]
            )
            ...
            for cycle in range(self.n_cycle):
                # TASK ON PROCESS
                if math.isnan(self.local_process_task[ue_index]["REMAIN"]) and (
                    not self.ue_computation_queue[ue_index].empty()
                ):
                    while not self.ue_computation_queue[ue_index].empty():
                        ...
                        if get_task["SIZE"] != 0:
                            if self.time_count - get_task["TIME"] + 1 <= self.max_delay:
                                self.local_process_task[ue_index]["UE_ID"] = get_task[
                                    "UE_ID"
                                ]
                                self.local_process_task[ue_index]["TASK_ID"] = get_task[
                                    "TASK_ID"
                                ]
                                self.local_process_task[ue_index]["SIZE"] = get_task[
                                    "SIZE"
                                ]
                                self.local_process_task[ue_index]["DENS"] = get_task[
                                    "DENS"
                                ]
                                self.local_process_task[ue_index]["TIME"] = get_task[
                                    "TIME"
                                ]
                                self.local_process_task[ue_index]["REMAIN"] = (
                                    self.local_process_task[ue_index]["SIZE"]
                                )
                                self.local_process_task[ue_index]["DIV"] = get_task[
                                    "DIV"
                                ]
                                ...
                                self.process_delay[get_task["TIME"], ue_index] = (
                                    self.max_delay
                                )
                ...
                if self.local_process_task[ue_index]["REMAIN"] > 0:
                    ...
                    if self.local_process_task[ue_index]["REMAIN"] >= (
                        ue_comp_cap / self.local_process_task[ue_index]["DENS"]
                    ):

                        self.ue_bit_processed[
                            self.local_process_task[ue_index]["TIME"], ue_index
                        ] += (ue_comp_cap / self.local_process_task[ue_index]["DENS"])
                        self.ue_comp_energy[
                            self.local_process_task[ue_index]["TIME"], ue_index
                        ] += (
                        ...
                        self.ue_bit_processed[
                            ...
                            self.local_process_task[ue_index]["REMAIN"]
                            / self.local_process_task[ue_index]["DENS"]
                        )
                        self.ue_comp_energy[
                            self.local_process_task[ue_index]["TIME"], ue_index
                        ] += (
                    ...
                    if self.local_process_task[ue_index]["REMAIN"] <= 0:
                        self.process_delay[
                            ...
                            self.time_count
                            - self.local_process_task[ue_index]["TIME"]
                            + 1
                        )
                ...
                if ue_arrive_task_size != 0:
                    tmp_tilde_t_ue_comp = np.max(
                        [self.t_ue_comp[ue_index] + 1, self.time_count]
                    )
                    self.t_ue_comp[ue_index] = np.min(
                        [
                            tmp_tilde_t_ue_comp
                            + math.ceil(
                                ue_arrive_task_size
                                * ue_action_local[ue_index]
                                / (ue_comp_cap / ue_arrive_task_dens)
                            )
                            - 1,
            ...
            for edge_index in range(self.n_edge):
                ...
                for cycle in range(self.n_cycle):
                    ...
                    if math.isnan(
                        ...
                        not self.edge_computation_queue[ue_index][edge_index].empty()
                    ):
                        while not self.edge_computation_queue[ue_index][
                            edge_index
                        ].empty():
                            get_task = self.edge_computation_queue[ue_index][
                                edge_index
                            ].get()

                            if self.time_count - get_task["TIME"] + 1 <= self.max_delay:
                                self.edge_process_task[ue_index][edge_index][
                                    "UE_ID"
                                ] = get_task["UE_ID"]
                                self.edge_process_task[ue_index][edge_index][
                                    "TASK_ID"
                                ...
                                self.edge_process_task[ue_index][edge_index]["DENS"] = (
                                    get_task["DENS"]
                                )
                                self.edge_process_task[ue_index][edge_index]["TIME"] = (
                                    get_task["TIME"]
                                )
                                self.edge_process_task[ue_index][edge_index][
                                    "REMAIN"
                                ] = self.edge_process_task[ue_index][edge_index]["SIZE"]
                                self.edge_process_task[ue_index][edge_index]["DIV"] = (
                                    get_task["DIV"]
                                )
                                break
                            else:
                    ...
                    if self.edge_process_task[ue_index][edge_index]["REMAIN"] > 0:
                        ...
                        if self.edge_process_task[ue_index][edge_index]["REMAIN"] >= (
                            edge_cap
                            / self.edge_process_task[ue_index][edge_index]["DENS"]
                            / self.edge_ue_m[edge_index]
                        ):
                            self.edge_comp_energy[
                                self.edge_process_task[ue_index][edge_index]["TIME"],
                                ue_index,
                                edge_index,
                            ] += (
                                ...
                                self.edge_p_comp * self.duration
                            )
                        ...
                        if self.edge_process_task[ue_index][edge_index]["REMAIN"] <= 0:
                            ...
                            # self.task_history[self.edge_process_task[ue_index][edge_index]['UE_ID']][self.edge_process_task[ue_index][edge_index]['TASK_ID']]['d_state'][self.edge_process_task[ue_index][edge_index]['DIV']] = -1
                            self.edge_drop[ue_index, edge_index] = (
                                self.edge_process_task[ue_index][edge_index]["REMAIN"]
                            )
                            self.process_delay[
                                self.edge_process_task[ue_index][edge_index]["TIME"],
        ...
        for ue_index in range(self.n_ue):
            ...
            for cycle in range(self.n_cycle):
                ...
                # TASK ON PROCESS
                if math.isnan(self.local_transmit_task[ue_index]["REMAIN"]) and (
                    not self.ue_transmission_queue[ue_index].empty()
                ):
                    while not self.ue_transmission_queue[ue_index].empty():
                        ...
                        if get_task["SIZE"] != 0:
                            ...
                            if self.time_count - get_task["TIME"] + 1 <= self.max_delay:
                                self.local_transmit_task[ue_index]["UE_ID"] = get_task[
                                    "UE_ID"
                                ]
                                self.local_transmit_task[ue_index]["TASK_ID"] = (
                                    get_task["TASK_ID"]
                                )
                                self.local_transmit_task[ue_index]["SIZE"] = get_task[
                                    "SIZE"
                                ]
                                self.local_transmit_task[ue_index]["DENS"] = get_task[
                                    "DENS"
                                ]
                                self.local_transmit_task[ue_index]["TIME"] = get_task[
                                    "TIME"
                                ]
                                self.local_transmit_task[ue_index]["EDGE"] = int(
                                    get_task["EDGE"]
                                )
                                self.local_transmit_task[ue_index]["REMAIN"] = (
                                    self.local_transmit_task[ue_index]["SIZE"]
                                )
                                self.local_transmit_task[ue_index]["DIV"] = get_task[
                                    "DIV"
                                ]
                                break
                            else:
                ...
                if self.local_transmit_task[ue_index]["REMAIN"] > 0:
                    ...
                    if (
                        ...
                        self.ue_tran_energy[
                            self.local_transmit_task[ue_index]["TIME"], ue_index
                        ] += (
                            ue_tran_cap[self.local_transmit_task[ue_index]["EDGE"]]
                            * self.ue_p_tran
                        )
                        self.ue_bit_transmitted[
                            self.local_transmit_task[ue_index]["TIME"], ue_index
                        ] += self.local_transmit_task[ue_index]["REMAIN"]
                    ...
                    if self.local_transmit_task[ue_index]["REMAIN"] <= 0:
                        ...
                        self.edge_computation_queue[ue_index][
                            self.local_transmit_task[ue_index]["EDGE"]
                        ].put(tmp_dict)
                        ...
                        self.process_delay_trans[
                            ...
                            self.time_count
                            - self.local_transmit_task[ue_index]["TIME"]
                            + 1
                        )
                    ...
                    # OTHER INFO
                if ue_arrive_task_size != 0:
                    tmp_tilde_t_ue_tran = np.max(
                        [self.t_ue_tran[ue_index] + 1, self.time_count]
                    )
                    self.t_ue_comp[ue_index] = np.min(
                        [
                            tmp_tilde_t_ue_tran
                            + math.ceil(
                                ue_arrive_task_size
                                * (1 - ue_action_local[ue_index])
                                / ue_tran_cap[ue_action_offload[ue_index]]
                            )
                            - 1,
                            self.time_count + self.max_delay - 1,
                        ]
        ...
        if self.time_count >= self.n_time:
            ...
            for time_index in range(self.n_time):
                for ue_index in range(self.n_ue):
                    if (
                        self.process_delay[time_index, ue_index] == 0
                        and self.arrive_task_size[time_index, ue_index] != 0
                    ):
                        self.process_delay[time_index, ue_index] = (
