...
class DuelingDoubleDeepQNetwork:
    ...
    def __init__(
        ...
        dueling=True,
        double_q=True,
        N_L1=120,  # Set dueling FC trunk to [120,80] (paper)
        N_lstm=80,
    ):

        # Move tf.reset_default_graph() to constructor start
        ...
        self.batch_size = (
            batch_size  # select self.batch_size number of time sequence for learning
        )
        ...
        t_params = tf.get_collection(
            "target_net_params"
        )  # obtain the parameters in target_net
        e_params = tf.get_collection(
            "eval_net_params"
        )  # obtain the parameters in eval_net
    ...
    def _build_net(self):
        ...
        def build_layers(
            s, lstm_s, c_names, n_l1, n_lstm, w_initializer, b_initializer
        ):
            ...
            with tf.variable_scope("l0"):
                ...
                lstm_output, lstm_state = tf.nn.dynamic_rnn(
                    lstm_dnn, lstm_s, dtype=tf.float32
                )
                lstm_output_reduced = tf.reshape(
                    lstm_output[:, -1, :], shape=[-1, n_lstm]
                )
            ...
            with tf.variable_scope("l1"):
                ...
                l1 = tf.nn.relu(
                    tf.matmul(tf.concat([lstm_output_reduced, s], 1), w1) + b1
                )
    ...
    def store_transition(self, s, lstm_s, a, r, s_, lstm_s_):
        ...
        transition = np.hstack(
            (s, [a, r], s_, lstm_s, lstm_s_)
        )  # stack in horizontal direction
    ...
    def choose_action(self, observation, action_mask=None):
        ...
        if np.random.uniform() < self.epsilon:
            ...
            if action_mask is not None:
                # set illegal actions to very low value
                actions_value = actions_value + (1.0 - action_mask) * -1e9

            self.store_q_value.append(
                {"observation": observation, "q_value": actions_value}
            )
    ...
    def load_model(self, iot):
        latest_ckpt = tf.train.latest_checkpoint(
            "./TrainedModel_20UE_2EN_PerformanceMode/800/" + str(iot) + "_X_model"
        )
